...
txt_2 = requests.get(f'https://www.telefonnyjdovidnyk.com.ua/nomer/{num}',headers={'User-Agent':random.choice()})
if txt_2.status_code == 200:
    ...
    for rev in range(1 if txt_2 == 1 else txt_2):
        txt_3 = BeautifulSoup(requests.get(f'https://www.telefonnyjdovidnyk.com.ua/nomer/{num}/p/{rev}',headers={'User-Agent':random.choice()}).text, 'html.parser').find_all('p', class_="comment-text")
        for txt in txt_3:
            review_arr.add(txt.get_text(strip=True))
num_oper = ['intertelecom','kyivstar','lifecell','vodafone','3mob']
for oper in num_oper:
    txt_4 = requests.get(f'https://ua.whokla.com/{oper}/{num}',headers={'User-Agent':random.choice()})
...
for filt in review_arr:
    ...
    if filt != '':
        comment_score_arr[f'{filt}'] = score_ai.predict([filt])